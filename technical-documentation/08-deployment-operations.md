...
## Environment variables (local dev — `.env`, copy from `.env.example`)
...
| `DATABRICKS_LLM_ENDPOINT` | No | Model Serving endpoint name for both AI features. Unset → both 503 with a clear "not configured" message; everything else works fine |
